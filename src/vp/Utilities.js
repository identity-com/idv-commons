...
const replaceUCA = (state, oldUCA, newUCA) => {
  const replaceDependencies = (uca) => {
    ...
    return {
      ...
      dependsOn: uca.dependsOn.map((dependency) => {
        const newDependentUCA = R.equals(dependency.uca.name, oldUCA.name)
          ? newUCA
          : replaceDependencies(dependency.uca);
  ...
  const newUCAs = R.map(
    (existingUCA) => (R.equals(existingUCA, oldUCA) ? newUCA : replaceDependencies(existingUCA)),
    state.ucas,
  );
