...
function checkUCAIsUpdateable(uca, processState) {
  ...
  if (!uca.status) {
    throw new UCAUpdateError(
      ...
      IDVErrorCodes.ERROR_IDV_UCA_UPDATE_NO_STATUS,
      uca.name,
    );
  }
  ...
  if (!processState.status) {
    throw new UCAUpdateError(
      'Unable to determine if the UCA can be updated, because the process it belongs to has no status',
      IDVErrorCodes.ERROR_IDV_UCA_UPDATE_NO_PROCESS_STATUS,
      uca.name,
    );
  }
  ...
  if (processState.status === ValidationProcessStatus.FAILED || processState.status === ValidationProcessStatus.CANCELED) {
    throw new UCAUpdateError(
      `Cannot update a UCA on a process in status ${processState.status}`,
      IDVErrorCodes.ERROR_IDV_PROCESS_HAS_FINAL_STATUS,
      uca.name,
    );
  }
  ...
  if ([UCAStatus.AWAITING_DEPENDENCY, UCAStatus.ACCEPTED, UCAStatus.TEMPLATE].includes(uca.status)) {
    throw new UCAUpdateError(
      `Cannot update a UCA in status ${uca.status}`,
      IDVErrorCodes.ERROR_IDV_UCA_HAS_FINAL_STATUS,
      uca.name,
    );
  }
...
class UCAHandler extends TypeHandler {
  ...
  canHandle(event, state) {
    ...
    const { ucaId } = event.payload;

    // throw an error if there are no UCAs in the process or if the UCA
    ...
    if (!this.ucaName) return true;

    // find the ids any UCAs in the state that have the name = this.ucaName
    const ucaIdsWithName = Object.keys(state.ucas).filter((aUCAId) => state.ucas[aUCAId].name === this.ucaName);

    // return true if the event is for one of the UCAs that has the right name
  ...
  async handle(state, event) {
    ...
    // throw an error if the UCA is not updateable
  ...
  validate(value, ucaId, ucaVersion = '1') {
    ...
    if (ucaVersion !== this.ucaVersion) {
      throw new UCAVersionError(
        `Handler ucaVersion '${this.ucaVersion}' doesn't equal state ucaVersion '${ucaVersion}'`,
        IDVErrorCodes.ERROR_IDV_UCA_WRONG_VERSION,
        this.ucaVersion,
        ucaVersion,
      );
      ...
      try {
        ...
        throw new UCAValueError(
          ...
          IDVErrorCodes.ERROR_IDV_UCA_BAD_VALUE,
          this.ucaName,
          value,
          error,
        );
   ...
   */
  // eslint-disable-next-line no-unused-vars
  async handleUCA(value, ucaState, processState) {
    // NOOP
  }
...
class ValidatingHandler extends UCAHandler {
  ...
  canHandle(event, state) {
    ...
    // Do not run the validating handler if the UCA has the clientHints parameters with prefill
...
class ExternalTaskHandler extends TypeHandler {
  ...
  canHandle(event, state) {
    ...
    // normally we wouldn't throw an error in canHandle, we would just skip the handler
